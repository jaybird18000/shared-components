...
#define PROGMEM
#endif
#define USE_HTML_VERSION 0

#if USE_HTML_VERSION == 1

#else

static const char kAppPageHtml[] = R"HTML(<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1" />
  <meta http-equiv="Cache-Control" content="no-store">
  <title>Boat Gen Control</title>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .control-container, .debug-container, .config-container {
      width: 100%;
      max-width: 480px;
      margin: 0 auto;
    }
    ...
    .button:active { background: inherit; opacity: 1; }
    ...
    .nav-button { color:#cbd8e6; background:none; border:none; font-size:0.95rem; cursor:pointer; }
    ...
    .input-flex input { flex:1; min-width:0; }
...
<body>
  ...
  <header>
    ...
    <h1>R33 Boat Controller (Master)</h1>
    <div class="header-statuses">
      <p id="socketStatus" class="socket-status disconnected">Browser: disconnected</p>
      <p id="slaveStatus" class="socket-status disconnected">Slave: disconnected</p>
    </div>
  </header>
  <main>
    <!-- CONTROL PAGE -->
    <section id="controlPage" class="page active">
      <div class="page-container control-container">
        ...
        <div class="card">
          <div class="status-row"><span class="status-label">AC Voltage</span><span id="acVoltage" class="status-value">--</span></div>
          <div class="status-row"><span class="status-label">AC Current</span><span id="acCurrent" class="status-value">--</span></div>
          <div class="status-row"><span class="status-label">AC Frequency</span><span id="acFrequency" class="status-value">--</span></div>
        </div>

        <div class="card">
          <div class="status-row"><span class="status-label">Generator Valve</span><span id="genValveStatus" class="status-value">unknown</span></div>
          <button class="button secondary" onclick="toggleGenValve()">Toggle Generator Valve</button>
        </div>

        <div class="card">
          <div class="status-row"><span class="status-label">AC Valve</span><span id="acValveStatus" class="status-value">unknown</span></div>
          <button class="button secondary" onclick="toggleAcValve()">Toggle AC Valve</button>
        </div>
      ...
      <div class="page-container config-container">
        ...
        <div id="configTab" class="config-tab-content active">
          ...
          <div class="card">
            ...
            <div class="input-row">
              <label for="ap_ipAddress">AP WiFi ADDRESS</label>
              <input id="ap_ipAddress" type="text" placeholder="x.x.x.x" />
            </div>

            <div class="input-row">
              <label for="ap_gateway">AP WiFi GATEWAY</label>
              <input id="ap_gateway" type="text" placeholder="x.x.x.x" />
            </div>

            <div class="input-row">
              <label for="ap_netmask">AP WiFi NETMASK</label>
              <input id="ap_netmask" type="text" placeholder="x.x.x.x" />
            </div>
          </div>
        </div>
...
<script>
    ...
    function setSlaveStatus(text) {
      const status = document.getElementById('slaveStatus');
      if (!status) return;
      status.textContent = 'Slave: ' + String(text || 'disconnected').toUpperCase();
      status.className = 'socket-status';
    ...
    function setAPwifi(ssid_text, pw_text, ipAddress, gateway, netmask) {
      if (ssid_text) document.getElementById('ap_ssid').value = ssid_text;
      if (pw_text) document.getElementById('ap_password').value = pw_text;
      if (ipAddress) document.getElementById('ap_ipAddress').value = ipAddress;
      if (gateway) document.getElementById('ap_gateway').value = gateway;
      if (netmask) document.getElementById('ap_netmask').value = netmask;
    }
    ...
    function tryToSubscribe() {
      if (domReady && socketReady) {
        setTimeout(() => {
          sendMessage({command:'subscribe_debug', lastDebugId:lastDebugId});
        }, 200);   // 200–300 ms works reliably on iPhone
      }
    }

    /* ❌ THIS WAS REMOVED IN PART 3 (swipe fix)
    ...
    */
    ...
    function sendMessage(payload) {
      if (socket && socket.readyState === WebSocket.OPEN) {
       ...
       (Only change made to your original file)
    --------------------------------------------------------- */
    (function() {
    ...
    function handleServerMessage(payload) {
      if (payload.type === 'status') {
        ...
        setStatusText('genValveStatus', payload.generatorValve || 'unknown');
        setStatusText('acValveStatus', payload.acValve || 'unknown');
        setSocketStatus('connected', payload.ssid || '');
        if (payload.slaveConnection) {
          setSlaveStatus(payload.slaveConnection);
        }
      ...
      if (payload.type === 'AP_wifi_config')
        setAPwifi(payload.ssid, payload.password, payload.ipAddress,
                  payload.gateway, payload.netmask);
    ...
    /* ---------------------------------------------------------
       UPDATED START/STOP FUNCTIONS (IMMEDIATE STATUS CHANGE)
    --------------------------------------------------------- */
    function startGenerator() {
      ...
      setStatusText("generatorState", "STARTING");
    ...
    /* ---------------------------------------------------------
       VALVE + WIFI CONFIG
    --------------------------------------------------------- */
    function toggleGenValve() { sendMessage({command:'toggle_gen_valve'}); }
    function toggleAcValve() { sendMessage({command:'toggle_ac_valve'}); }
    ...
    function saveAPWifiConfig() {
      ...
      const ipAddress = document.getElementById('ap_ipAddress').value;
      const gateway = document.getElementById('ap_gateway').value;
      const netmask = document.getElementById('ap_netmask').value;

      if (ipAddress && !isValidIPv4(ipAddress)) {
        appendLog('Invalid IP address format');
        return;
      }
      if (gateway && !isValidIPv4(gateway)) {
        appendLog('Invalid gateway format');
        return;
      }
      if (netmask && !isValidIPv4(netmask)) {
        appendLog('Invalid netmask format');
        return;
      }
      ...
      sendMessage({
        ...
        ssid, password, ipAddress, gateway, netmask
      });
    }
