#ifndef SUNSHADE_PAGES_H
#define SUNSHADE_PAGES_H

#ifndef PROGMEM
#define PROGMEM
#endif

static const char kSunShadePageHtml[] PROGMEM = R"HTML(
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
...
<body>
  ...
  <header>
    ...
    <h1>R33 Sun Shade Controller</h1>
      ...
      <div class="page-container control-container">
        ...
        <div class="card">
          <div class="status-row">
            <span class="status-label">Shade State</span>
            <span id="shadeState" class="status-value">unknown</span>
          </div>

          <button class="button" id="shadeOpenButton" onclick="openShade()">Open</button>
          <button class="button secondary" id="shadeCloseButton" onclick="closeShade()">Close</button>
        </div>

        <div class="card">
          <div class="status-row">
            <span class="status-label">Position</span>
            <span id="shadePercentage" class="status-value">--</span>
          </div>
          <div class="progress-bar">
            <div id="shadeProgressFill" class="progress-bar-fill"></div>
          </div>
        </div>
  ...
  <script>
    ...
    window.addEventListener('DOMContentLoaded', () => {
      const openBtn  = document.getElementById("shadeOpenButton");
      const closeBtn = document.getElementById("shadeCloseButton");

      resetButton(openBtn, "Open", true);
    ...
    /* ---------------------------------------------------------
       UPDATED SHADE BUTTON LOGIC
    --------------------------------------------------------- */
    let openTimeout = null;
    let closeTimeout  = null;

    function updateShadeButtons(state) {
      const openBtn  = document.getElementById("shadeOpenButton");
      const closeBtn = document.getElementById("shadeCloseButton");
      ...
      if (pendingOpen) {
        if (state === "OPEN") {
          ...
          resetButton(closeBtn, "Close", false);
        }
        if (state === "OPENING") {
          pendingOpen = false;
          if (openTimeout) clearTimeout(openTimeout);
      ...
      if (state === "CLOSED") {
        ...
        resetButton(closeBtn, "Close", true);
      }
      else if (state === "OPENING") {
        setButtonLoading(openBtn, "Opening...");
        closeBtn.disabled = true;
        closeBtn.classList.add("secondary");
      }
      else if (state === "CLOSING") {
        setButtonLoading(closeBtn, "Closing...");
        openBtn.disabled = true;
        openBtn.classList.add("secondary");
      }
      else {
    ...
    function handleServerMessage(payload) {
      if (payload.type === 'status') {
        ...
        const shadeState = payload.shadeState || 'UNKNOWN';
        lastKnownState = shadeState;
        ignoreShadeState = false;

        if (pendingClose && shadeState === "OPEN") ignoreShadeState = true;
        if (pendingOpen && shadeState === "CLOSED") ignoreShadeState = true;

        if (!ignoreShadeState)
          setStatusText('shadeState', shadeState);

        updateShadeButtons(shadeState);

        setSocketStatus('connected', payload.ssid || '');

        const percentage = payload.shadePosition;
        if (percentage !== undefined && percentage !== null) {
          const pct = Math.min(100, Math.max(0, percentage));
          document.getElementById('shadePercentage').textContent = pct + '%';
          document.getElementById('shadeProgressFill').style.width = pct + '%';
        }
    ...
    function openShade() {
      ...
      document.getElementById("shadeState").textContent = "OPENING";

      sendMessage({command:'open_shade'});
      ...
      openTimeout = setTimeout(() => {
        if (pendingOpen) {
          ...
          setStatusText("shadeState", previousState);
    ...
    function closeShade() {
      ...
      setStatusText("shadeState", "CLOSING");

      sendMessage({command:'close_shade'});
